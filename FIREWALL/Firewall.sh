...
# Instructions:
 ...
 # 2 - restart rsyslog: /etc/init.d/rsyslog
...
# Masquerading
iptables -A FORWARD --in-interface enp0s3 -j ACCEPT
...
iptables -A ICMPSCAN -m recent --update --seconds 1 --hitcount 2 --name badicmp --rsource -j LOG --log-level 6 --log-prefix "WARNING DEATH PING ATTACK!!!:" 
iptables -A ICMPSCAN -m recent --update --seconds 1 --hitcount 2 --name badicmp --rsource -j DROP
iptables -A ICMPSCAN -m recent --update --seconds 1 --hitcount 1 --name badicmp --rsource -j ACCEPT

# Protecting portscans
# These rules add scanners to the portscan list, and log the attempt.
iptables -A INPUT -p tcp -m multiport --dport 0:65535 -m recent --name portscan --set -j LOG --log-level 6 --log-prefix "WARNING PORTSCAN ATTACK!!!:"
iptables -A INPUT -p tcp -m multiport --dport 0:65535 -m recent --name portscan --set -j DROP
# These rules add scanners to the portscan list, and log the attempt.
iptables -A INPUT -p udp -m multiport --dport 0:65535 -m recent --name portscan --set -j LOG --log-level 6 --log-prefix "WARNING PORTSCAN ATTACK!!!:"
iptables -A INPUT -p udp -m multiport --dport 0:65535 -m recent --name portscan --set -j DROP

iptables -A INPUT -p tcp --dport 22 -i $NIC -j ACCEPT
